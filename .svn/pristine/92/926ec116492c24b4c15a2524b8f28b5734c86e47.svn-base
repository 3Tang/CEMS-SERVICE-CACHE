...
public class SqlModel extends ArrayList<ModelField>{
	...
	public SqlModel(Class<?> clazz,Long id){
	...
	public SqlModel(Class<?> clazz,Integer id){
	 ...
	 * @param clazz 实体类型。
	 */
	public void loadModel(Class<?> clazz){
		//设置表名称
		...
		for(Field field : fields){
			if(!field.isAnnotationPresent(IgnoreColumn.class)){
				int type= ModelParse.isNumber(field)?1:0;
				add(new ModelField(field.getName(),ModelParse.getColumnName(field),type,null));
			}
		}
		size=size();
	}
